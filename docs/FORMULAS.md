...
cruza o limiar de 10% sem o benefício e não cruza com ele, continuidade dentro do
limite, e favela mapeada. Onde ele não serve, o exemplo diz qual município usa e por
quê.
...
**E aqui está a leitura que o aplicativo transforma em encaminhamento:** em São João
de Meriti o peso cruza o limiar de 10% **sem** o benefício e não cruza **com** ele. A
mesma família, elegível, paga acima do limiar se estiver fora e abaixo se estiver
dentro. Não é caso de discutir tarifa: o instrumento que resolve é o cadastro.
...
| `12-12` | Selecao da vigencia na data de referencia e descarte de tarifa nao po… | limiar | O cadastro da ANEEL guarda o historico completo de reajustes, com um par inicio/fim por revisao; a interseccao fechada dos dois limites na data de re… **Risco:** Comparacao com NaT retorna False: qualquer registro com DatFimVigencia ausente ou nao parseavel (vigencia em aberto) e eliminado, e distribuidoras cu… |
...
| `16-17` | Recorte do ano de referencia no cadastro comercial INDGER | limiar | O INDGER e uma serie com periodicidade por data de referencia; o filtro por ano isola 2024, mesmo ano da data tarifaria de corte (2024-12-01, linha 1… **Risco:** O filtro e por ano inteiro, enquanto a tarifa e de um unico dia (01/12/2024): as duas pontas do calculo nao estao no mesmo instante. Linhas com data … |
...
| `11-11` | Corte temporal na data de referencia e descarte de tarifa nao positiva | limiar | Congela um instante unico para que todas as distribuidoras sejam comparadas na mesma data, e nao em revisoes tarifarias de meses diferentes. A data 2… **Risco:** Uma distribuidora sem vigencia cobrindo exatamente essa data desaparece do calculo. Linha com ini ou fim ilegivel, virada NaT na linha 9, tambem cai … |
...
| `13-14` | Recorte do cadastro comercial ao ano de 2024 | leitura | Mantem todas as competencias do INDGER informadas em 2024, alinhando o cadastro comercial ao ano da foto tarifaria. Identico a 02_tarifa.py linha 18. **Risco:** O filtro por ano preserva mais de uma competencia por par municipio-CNPJ, e a soma da linha 17 vai acumula-las (ver a formula seguinte). Data ausente… |
...
| `14-14` | Filtro do ano de referencia do INDGER | limiar | Alinha o ano do universo de unidades consumidoras com o ano da tarifa de referencia, dezembro de 2024. O codigo nao registra essa justificativa, o al… **Risco:** Mantem os doze meses de 2024, nao um mes unico. A soma da linha 17 passa a ser uma soma de estoques mensais, nao um estoque. Isso nao distorce a medi… |
...
| `21-21` | Constantes dos tetos de renda das faixas do CadUnico | regra normativa | Sao os dois tetos de renda per capita que delimitam as faixas usadas no resto do arquivo, meio salario minimo e a linha de pobreza. O codigo nao regi… **Risco:** Constantes fixas no codigo, sem ano nem referencia. Se o script for reexecutado em outro exercicio sem atualizar os valores, os pesos calculados pass… |
...
| `6-7` | Recorte temporal do INDGER para 2024 | leitura | Fixa o ano de referencia em 2024, o mesmo ano usado por 02_tarifa.py e 06c para atribuir tarifa por municipio. **Risco:** errors='coerce' converte data invalida em NaT, e NaT.dt.year e NaN, entao a linha e descartada em silencio — nenhuma contagem de perdas e impressa. O… |
...
| `7-7` | Conversao da data de referencia e recorte do ano de 2024 | limiar | Alinha a atribuicao de distribuidora ao mesmo ano de referencia usado nos demais passos do pipeline que tambem filtram 2024 (por exemplo 06c, que cas… **Risco:** errors="coerce" transforma data invalida em NaT, e NaT nao satisfaz ano==2024, entao o registro e descartado em silencio, sem contagem nem aviso. O f… |
...
| `17` | Posicao nacional da tarifa | estatística | ascending=False faz 1 ser a tarifa mais cara, como DICIONARIO.md descreve. **Risco:** astype(int) levanta ValueError se houver qualquer NaN em tarifa_municipal: o script so roda porque a coluna esta completa nesse ponto, e passa a falh… |
| `17-17` | Posicao do municipio no ranking de tarifa cheia | estatística | Ordena do mais caro para o mais barato; method="min" da a todos os empatados o menor posto do bloco, de modo que a posicao numero 1 seja sempre a tar… **Risco:** rank ignora nulos e devolve NaN para eles; o astype(int) sobre uma coluna com NaN levanta erro de conversao e interrompe o script. Ou seja, este pass… |
| `18` | Indicador de violacao do limite de continuidade | limiar | d3_corr e razao entre apurado e limite regulatorio, entao 1,00 e a fronteira; o np.where preserva vazio para os municipios sem apuracao em vez de os … **Risco:** Sem o np.where, (d3_corr>=1) com NaN daria False e transformaria 'sem dado' em 'sem violacao' — a escolha e justamente evitar isso, e o mesmo cuidado… |
| `18-18` | Bandeira de violacao do limite de continuidade | limiar | d3_corr vem de 03b como media, ponderada por conjunto eletrico, da razao entre o indicador de continuidade apurado e o respectivo limite regulatorio;… **Risco:** Como d3_corr e media ponderada entre conjuntos, um municipio pode ficar abaixo de 1 no agregado tendo conjuntos individualmente violados (a coluna d3… |
| `19-22` | Bandeira de sobrecobertura na mesma base da lacuna | limiar | lacuna_bruta_casada e definida em 06d linha 54 como familias_elegiveis_mar26 menos beneficiarios_mar26, e lacuna_pos e a mesma diferenca truncada em … **Risco:** O comparador < 0 trata valor nulo como falso, de modo que municipio sem lacuna_bruta_casada sai com sobrecobertura igual a zero, indistinguivel de mu… |
| `23-23` | Bandeira de subsidio indisponivel | limiar | subs_mar26 e o valor de subsidio da competencia de marco de 2026, renomeado em 06d linha 37 a partir de subs_2026_03, e serve de denominador em 06d l… **Risco:** O comparador <= 0 e falso para nulo, entao municipio sem subs_mar26 sai com a bandeira em zero, isto e, como se o subsidio estivesse disponivel. A co… |
| `24-30` | Definicao das colunas da tabela final | serialização | Fixa quais grandezas saem para o app e em que ordem, descartando as demais colunas intermediarias de ipem_v5. A lista mistura deliberadamente as duas… **Risco:** Qualquer nome ausente em df derruba o script com KeyError na linha 31, que e o unico mecanismo de verificacao do contrato. Colunas usadas no calculo … |
| `31-31` | Materializacao da tabela de saida | serialização | Seleciona e copia, para que a tabela exportada seja independente de df. A selecao por lista e o ponto em que a ausencia de qualquer coluna esperada s… **Risco:** A copia nao valida tipos nem intervalos; colunas nulas passam intactas ate o CSV. Se cols contiver nome repetido, a coluna sai duplicada no arquivo. |
| `32-32` | Diagnostico impresso de numero de linhas | apresentação | Permite conferir a olho se a contagem bate com o universo de municipios esperado. **Risco:** Nenhuma comparacao automatica com o numero esperado de municipios; o script segue e grava mesmo se a contagem estiver errada. |
| `33-33` | Diagnostico impresso de nulos por coluna | apresentação | Lista somente as colunas com faltantes, encurtando a saida e expondo onde o casamento de bases falhou. isna().sum() e calculado duas vezes na mesma e… **Risco:** E somente impressao: nenhum limite de nulos e imposto e a gravacao ocorre em seguida de qualquer forma. Colunas cujo faltante foi convertido em zero … |
| `34-34` | Gravacao do CSV consumido pelo app | serialização | index=False evita uma coluna extra de indice sem significado. O arquivo e a entrada dos passos de payload e validacao (09_payload.py e os scripts val… **Risco:** Sobrescreve a saida anterior sem versionar nem checar se os diagnosticos impressos acima eram aceitaveis. A gravacao usa o padrao do pandas para sepa… |
| `35-35` | Mensagem final de conclusao | apresentação | Sinaliza o fim do passo para o orquestrador e para quem le o log. **Risco:** O OK indica apenas que nenhuma excecao ocorreu; nao afirma nada sobre cobertura da distribuidora, quantidade de nulos ou coerencia temporal entre as … |
...
| `17-17` | Peso da conta com beneficio sobre a renda domiciliar na faixa de pobr… | aritmética | O denominador reconstroi a renda domiciliar no TETO da faixa de pobreza: renda per capita maxima da faixa multiplicada pelo tamanho medio do domicili… **Risco:** Divisao por zero se moradores_por_domicilio for 0 (produz inf em pandas, sem erro) e NaN se a coluna faltar. Usar o TETO da faixa como renda da famil… |
...
| linhas | operação | espécie | o que faz, e o que a tornaria errada |
|---|---|---|---|
| `21-21` | Montagem do caminho base da ANEEL | leitura | RAW e definido em _paths.py como um caminho absoluto de maquina local (C:/Users/diego/Desktop/IndiceMunicipalPobrezaEnergetica - IMPE/data/raw), fora… **Risco:** Concatenacao textual em vez de Path: se RAW terminar com barra, o caminho fica com barra dupla. Como RAW aponta para uma pasta que nao acompanha o re… |
...
| `2433-2446` | Pintura do mapa e montagem da legenda | cartografia | O nulo e testado antes da funcao de cor, entao ausencia de dado nunca passa pelo classificador. A legenda e gerada da mesma rampa que pinta o mapa, e… **Risco:** O emparelhamento por indice supoe que labels e ramp tenham o mesmo comprimento; se labels for maior, as ultimas amostras ficam com fundo indefinido, … |
| `2448-2453` | Contorno do municipio selecionado | cartografia | Limpa e redesenha em uma camada separada, acima das demais, para que o contorno da selecao nao dependa do estilo de preenchimento da metrica. A busca… **Risco:** A busca linear percorre a malha inteira a cada chamada, e paintMap chama markSel a cada troca de metrica; com 5.570 feicoes isso repete a varredura t… |
| `2455-2459` | Enquadramento no municipio selecionado | cartografia | A folga de 70 pixels mantem o entorno visivel em vez de encher a tela com o poligono, e o teto de zoom 10 impede que um municipio pequeno leve o mapa… **Risco:** O try vazio engole qualquer falha de getBounds, inclusive geometria vazia, e o mapa fica onde estava sem nenhuma indicacao. Municipio com territorio … |
| `2462-2467` | Valor exibido na dica do mapa | apresentação | Usa a mesma funcao get que pintou o poligono, entao a cor e o numero da dica vem da mesma conta, e o formatador de cada metrica anexa a unidade. O nu… **Risco:** A dica e montada por innerHTML a cada passagem do ponteiro, com escape aplicado a nome e distribuidora mas nao a D.uf[i], que e interpolado direto. O… |
| `2469-2476` | Posicionamento da dica dentro da janela | apresentação | Desloca a dica 14 pixels do cursor e a espelha para o outro lado quando ela passaria da borda, com margem de 8 pixels. As reservas de 190 e 70 pixels… **Risco:** O espelhamento nao verifica se a posicao espelhada tambem fica fora da tela: perto do canto superior esquerdo, a dica pode ir para coordenada negativ… |
| `2515-2519` | Tabela equivalente ao mapa, ordenada pela metrica | agregação | Reproduz em texto o conteudo do mapa para a mesma metrica selecionada, filtrando os nulos e contando quantos ficaram de fora, numero que a legenda da… **Risco:** A funcao get e chamada duas vezes por comparacao dentro do sort, ou seja, da ordem de N log N chamadas para 5.570 municipios, recalculando metricas d… |
